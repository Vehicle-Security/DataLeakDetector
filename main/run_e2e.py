"""
DataLeakDetector end-to-end pipeline.

Canonical architecture:
  1. EventCorrelator: logs, frontend windows, lineage, and evidence windows.
  2. FrameAnalyzer: video/VLM observations for suspicious windows.
  3. LeakReasoner: Datalog facts, leak-path reasoning, and evidence report.

Historical directories still provide implementation code, but this file should
describe and orchestrate the canonical three-module flow.
"""
...
from data_leak_detector.legacy_paths import (  # noqa: E402
    LEAK_REASONER_IMPL,
    add_pipeline_legacy_paths,
)
...
try:
    ...
    # Load root .env first; legacy implementations may discover it too.
    load_dotenv(os.path.join(PROJECT_DIR, '.env'))
    # Load LeakReasoner implementation .env with override.
    load_dotenv(str(LEAK_REASONER_IMPL / '.env'), override=True)
except ImportError:
    pass
...
def import_modules():
    ...
    global UploadEvent
    global EventCorrelator
    global DatalogEngine, PromptTemplates

    # FrameAnalyzer adapter: historical orchestration still
    # owns the expensive VLM frame pass, but it is now treated as the module-2
    # implementation detail behind the target architecture.
    try:
        from upload_detector_graph import create_upload_detector_graph
        from upload_detector_state import create_initial_state, UploadEvent
        from upload_detection_config import UploadDetectionConfig
        from log_first_detector import LogFirstDetector
        print("✅ FrameAnalyzer stage loaded")
    except ImportError as e:
        logger.warning(f"FrameAnalyzer adapter import failed: {e}")
        import traceback
        traceback.print_exc()
        create_upload_detector_graph = None
        create_initial_state = None
    ...
    try:
        ...
        print("✅ EventCorrelator stage loaded")
    except ImportError as e:
        logger.warning(f"EventCorrelator import failed: {e}")
        EventCorrelator = None

    # LeakReasoner implementation.
    try:
        from data_leak_detector.leak_reasoner import DatalogEngine, PromptTemplates
        print("✅ LeakReasoner stage loaded")
    except ImportError as e:
        logger.warning(f"LeakReasoner import failed: {e}")
        DatalogEngine = None
        PromptTemplates = None
...
def run_module3_pipeline(log_file: str, video_path: str,
    ...
    """
    Compatibility entry for the historical frame-observation pipeline.
    New orchestration should call run_frame_analyzer_stage().

    Returns:
        FrameAnalyzer observation and legacy detection result dictionary.
    """
    print("\n" + "=" * 80)
    print("🎬 FrameAnalyzer: frame/VLM observation stage")
    print("   implementation adapter: historical frame-observation pipeline")
    print("=" * 80)

    vlm_fallback_meta = None
    if create_upload_detector_graph is None:
        print("   ⚠️ FrameAnalyzer adapter not loaded; skipping")
        return {
            "alert_events": [],
            "info_events": [],
            "upload_events": [],
    ...
    # Historical adapter configuration.
    ...
    initial_state = create_initial_state(
        ...
        blacklist_apps=blacklist_apps,
        whitelist_apps=whitelist_apps,
        # search_duration uses the adapter default.
    )
    ...
    if not final_state:
        print("   ⚠️ FrameAnalyzer adapter returned no result")
        return {
            "alert_events": [],
            "info_events": [],
            "upload_events": [],
    ...
    print(f"\n   ✅ FrameAnalyzer observation stage complete")
    print(f"   📊 统计:")
...
def run_frame_analyzer_stage(log_file: str, video_path: str, index_path: str) -> Dict[str, Any]:
    """Run the canonical FrameAnalyzer stage.

    The current implementation still reuses the historical frame-observation
    adapter while the physical source tree is being migrated.
    """
    return run_module3_pipeline(log_file=log_file, video_path=video_path, index_path=index_path)
...
def _inject_connected_facts_from_module3(engine, module3_result: Dict[str, Any],
    ...
    print("\n   📎 injecting supplementary Datalog facts from correlated evidence...")
    ...
    if not direct_mappings:
        print("      📌 no lineage mappings found; inferring derived-file relationships from logs...")
    ...
    correlation_bundle = module3_result.get("correlation_bundle", {}) or {}
    for candidate in correlation_bundle.get("upload_candidates", []) or []:
        timestamp_str = str(candidate.get("timestamp", "") or "")
        timestamp_ms = _parse_timestamp_ms(timestamp_str)
        current_files = candidate.get("current_files", []) or []
        leak_file = _normalize_file_path(str(current_files[0] if current_files else candidate.get("original_file", "")))
        if not leak_file:
            continue

        event_app = str(candidate.get("app_name", "") or "unknown")
        matched_log = find_latest_log(
            leak_file,
            event_types=["file_upload", "upload_detected", "created", "modified"],
            timestamp_ms=timestamp_ms,
        )
        uploader_process = _normalize_process_name(
            (matched_log or {}).get("process_info", {}).get("process_name", event_app)
        )
        channel = str(candidate.get("sink_type", "") or "").lower() or infer_channel(
            (matched_log or {}).get("window_info", {}).get("window_title", "")
        )
        ...
        if add_leak_fact(
            ...
            timestamp_ms,
            timestamp_str,
            f"EventCorrelator: {uploader_process} leaked {os.path.basename(leak_file)} via {channel}",
        ):
            print(f"      + LeakFile({uploader_process}, {os.path.basename(leak_file)}) [EventCorrelator]")

        original_file = _normalize_file_path(str(candidate.get("original_file", "") or ""))
        owner_process = find_owner_process(leak_file, timestamp_ms, original_file)
        if owner_process and owner_process != uploader_process:
            if add_cross_fact(
                owner_process,
                uploader_process,
                ...
                timestamp_ms,
                timestamp_str,
                f"EventCorrelator: {owner_process} -> {uploader_process} ({os.path.basename(leak_file)})",
            ):
                print(f"      + CrossProcessTransfer({owner_process} -> {uploader_process}) [EventCorrelator]")
...
def _inject_facts_from_module3(engine, module3_result: Dict[str, Any],
                                logs: List[Dict]) -> List:
    """
    从已关联证据中注入补充 Datalog 事实
    
    解决的问题: LLM生成的Datalog事实经常因为进程名/文件名不一致而无法
    形成连通的污点链。用已关联证据补充关键事实。
    ...
    # 5. 从日志中检测文件分片/派生模式
    #    If no lineage mappings exist, infer derived-file relationships from logs.
    if not direct_mappings:
        print("      📌 no lineage mappings found; inferring derived-file relationships from logs...")
...
def run_threat_detector(logs: List[Dict],
    ...
    """
    Compatibility entry for the historical leak-reasoning implementation.

    功能:
    1. 从已关联证据中构建视频帧分析数据
    2. 使用 LLM 生成 Datalog 事实
    3. 从已关联证据注入补充事实（确保污点链连通）
    4. 运行 Souffle/Python 推理
    5. 检测泄露路径
    """
    print("\n" + "=" * 80)
    print("⚖️ LeakReasoner: Datalog reasoning")
    print("=" * 80)

    if DatalogEngine is None:
        print("   ⚠️ LeakReasoner not loaded; skipping")
        return {"leak_paths": [], "datalog_facts": []}

    # Build frame-observation context for optional LLM fact generation.
    ...
    # If no frame observations are available, simulate minimal context from logs.
    if not video_frames:
        print("   ⚠️ no frame observations; simulating minimal frame context from logs...")
        for log in logs:
    ...
    try:
        ...
        print(f"   continuing with deterministic evidence only...")
    ...
    # Inject supplementary facts from correlated evidence.
...
def run_leak_reasoner_stage(logs: List[Dict], evidence_result: Dict[str, Any]) -> Dict[str, Any]:
    """Run the canonical LeakReasoner stage."""
    return run_threat_detector(logs=logs, module3_result=evidence_result)
...
def _build_video_frames_from_module3(module3_result: Dict[str, Any]) -> List[Dict]:
    ...
    数据来源:
    1. upload_events 的 extra_info（原始帧分析事件数据）
    ...
    for event in upload_events:
        ...
        if extra_info:
            # extra_info carries original frame-observation data.
...
def _get_event_value(event: Any, key: str, default: Any = "") -> Any:
...
def _collect_correlation_sensitive_files(
    logs: List[Dict[str, Any]],
    detection_result: Dict[str, Any],
) -> List[str]:
    candidates: List[str] = []
    candidates.extend(_as_list(detection_result.get("sensitive_files", [])))

    for key in ("upload_events", "alert_events", "info_events"):
        for event in detection_result.get(key, []) or []:
            candidates.append(_get_event_value(event, "original_file", ""))
            extra_info = _get_event_value(event, "extra_info", {}) or {}
            if isinstance(extra_info, dict):
                candidates.append(extra_info.get("original_file", ""))

    for record in detection_result.get("operation_records", []) or []:
        candidates.append(record.get("sensitive_file_path", ""))

    for log in logs:
        file_path = _normalize_file_path(str(log.get("file_path", "") or ""))
        file_name = str(log.get("file_name", "") or os.path.basename(file_path))
        if file_path and _is_sensitive_filename(file_name):
            candidates.append(file_path)

    deduped: List[str] = []
    seen = set()
    for item in candidates:
        normalized = _normalize_file_path(str(item or ""))
        key = _file_identity_key(normalized)
...
def _frame_segments_from_detection_result(detection_result: Dict[str, Any]) -> List[Dict[str, Any]]:
    segments: List[Dict[str, Any]] = []
    seen = set()

    def add_segment(segment: Dict[str, Any]) -> None:
        key = (
            str(segment.get("time_range", "")),
            str(segment.get("app_name", "")),
            str(segment.get("operation_type", "")),
            str(segment.get("primary_resource", "")),
        )
        if key in seen:
            return
        seen.add(key)
    ...
    for index, event in enumerate(detection_result.get("upload_events", []) or []):
        extra_info = _get_event_value(event, "extra_info", {}) or {}
        if not isinstance(extra_info, dict):
            extra_info = {}

        timestamp = str(_get_event_value(event, "timestamp", "") or extra_info.get("timestamp", ""))
        time_range = str(
            extra_info.get("time_range")
            or _get_event_value(event, "time_range", "")
            or (f"{timestamp} - {timestamp}" if timestamp else "")
        )
        involved_timestamps = _as_list(
            extra_info.get("involved_timestamps")
            or _get_event_value(event, "involved_timestamps", [])
            or ([timestamp] if timestamp else [])
        )
        primary_resource = (
            extra_info.get("primary_resource")
            or extra_info.get("modified_filename")
            or extra_info.get("file_name")
            or _get_event_value(event, "upload_content", "")
            or _get_event_value(event, "file_path", "")
            or _get_event_value(event, "file_name", "")
        )
        related_resources = []
        for candidate_key in ("related_resources", "derived_files", "content_artifacts"):
            related_resources.extend(_as_list(extra_info.get(candidate_key)))
        original_file = _get_event_value(event, "original_file", "")
        if original_file:
            related_resources.append(original_file)

        add_segment(
            {
                "segment_id": f"det_upload_{index}",
                "time_range": time_range,
                "app_name": extra_info.get("app_name") or _get_event_value(event, "app_name", "unknown"),
                "operation_type": extra_info.get("operation_type") or _get_event_value(event, "operation_type", ""),
                "primary_resource": str(primary_resource or ""),
                "related_resources": [str(item) for item in related_resources if str(item or "").strip()],
                "action_description": (
                    extra_info.get("action_description")
                    or extra_info.get("description")
                    or _get_event_value(event, "description", "")
                ),
                "visible_evidence": [
                    str(item)
                    for item in [
                        _get_event_value(event, "file_name", ""),
                        _get_event_value(event, "upload_content", ""),
                        _get_event_value(event, "upload_content_mapping_link", ""),
                        extra_info.get("window_title", ""),
                    ]
                    if str(item or "").strip()
                ],
                "supporting_timestamps": [str(item) for item in involved_timestamps if str(item or "").strip()],
                "confidence": float(extra_info.get("confidence", 0.9) or 0.9),
            }
        )

    for index, record in enumerate(detection_result.get("operation_records", []) or []):
        timestamp = str(record.get("operation_time", "") or "")
        current_file = record.get("current_file") or record.get("sensitive_file_path", "")
        sensitive_file = record.get("sensitive_file_path", "")
        add_segment(
            {
                "segment_id": f"det_operation_{index}",
                "time_range": f"{timestamp} - {timestamp}" if timestamp else "",
                "app_name": record.get("app_name", "unknown"),
                "operation_type": record.get("operation", ""),
                "primary_resource": current_file,
                "related_resources": [item for item in [sensitive_file] if item and item != current_file],
                "action_description": record.get("description", "") or record.get("behavior_category", ""),
                "visible_evidence": [
                    str(item)
                    for item in [current_file, sensitive_file, record.get("behavior_category", "")]
                    if str(item or "").strip()
                ],
...
def _merge_correlation_bundle_into_detection_result(
    detection_result: Dict[str, Any],
    correlation_bundle: Dict[str, Any],
) -> Dict[str, Any]:
    detection_result["correlation_bundle"] = correlation_bundle

    file_mappings = detection_result.setdefault("file_mappings", {})
    direct = file_mappings.setdefault("direct_file_mappings", {})
    full = file_mappings.setdefault("full_file_mapping_chains", {})
    artifacts = file_mappings.setdefault("artifact_instances", [])
    lineage = correlation_bundle.get("file_lineage", {}) or {}

    direct.update(lineage.get("direct_file_mappings", {}) or {})
    full.update(lineage.get("full_file_mapping_chains", {}) or {})
    ...
    for artifact in lineage.get("artifact_instances", []) or []:
        artifact_id = str(artifact.get("artifact_id", ""))
        if artifact_id and artifact_id in artifact_seen:
            continue
        artifacts.append(artifact)
        if artifact_id:
            artifact_seen.add(artifact_id)

    operation_records = detection_result.setdefault("operation_records", [])
    record_seen = {
        (
            str(item.get("operation_time", "")),
            str(item.get("sensitive_file_path", "")),
    ...
    for record in correlation_bundle.get("operation_records", []) or []:
        enriched = dict(record)
        enriched.setdefault("source", "event_correlator")
        key = (
            str(enriched.get("operation_time", "")),
            str(enriched.get("sensitive_file_path", "")),
            str(enriched.get("current_file", "")),
            str(enriched.get("operation", "")),
        )
        if key in record_seen:
            continue
        operation_records.append(enriched)
        record_seen.add(key)

    _append_correlation_upload_events(detection_result, correlation_bundle)

    stats = detection_result.setdefault("statistics", {})
    bundle_stats = correlation_bundle.get("statistics", {}) or {}
    stats["event_correlator_status"] = correlation_bundle.get("analysis_status", "")
    stats["event_correlator_analysis_windows"] = bundle_stats.get("analysis_windows_output", 0)
    stats["event_correlator_correlated_events"] = bundle_stats.get("correlated_events_output", 0)
    stats["event_correlator_upload_candidates"] = bundle_stats.get("upload_candidates_output", 0)
    stats["event_correlator_lineage_direct_mappings"] = bundle_stats.get("lineage_direct_mappings", 0)
    stats["upload_events_detected"] = len(detection_result.get("upload_events", []) or [])
    stats["blacklist_alerts"] = len(detection_result.get("alert_events", []) or [])

    return detection_result


def _append_correlation_upload_events(
    detection_result: Dict[str, Any],
    correlation_bundle: Dict[str, Any],
) -> None:
    detection_result.setdefault("correlation_upload_candidates", [])
    detection_result["correlation_upload_candidates"] = correlation_bundle.get("upload_candidates", []) or []

    if UploadEvent is None:
        return

    config = UploadDetectionConfig() if UploadDetectionConfig is not None else None
    existing = {
        (
            _file_identity_key(_get_event_value(event, "file_path", "")),
            str(_get_event_value(event, "timestamp", ""))[:19],
            str(_get_event_value(event, "app_name", "")).lower(),
        )
        for event in detection_result.get("upload_events", []) or []
    }

    for candidate in correlation_bundle.get("upload_candidates", []) or []:
        current_files = candidate.get("current_files", []) or []
        file_path = _normalize_file_path(str(current_files[0] if current_files else candidate.get("original_file", "")))
        timestamp = str(candidate.get("timestamp", "") or "")
        app_name = str(candidate.get("app_name", "") or "unknown")
        key = (_file_identity_key(file_path), timestamp[:19], app_name.lower())
        if not file_path or key in existing:
            continue

        app_category = config.get_app_category(app_name) if config is not None else "unknown"
        should_alert = app_category != "whitelist"
        alert_level = "critical" if app_category == "blacklist" else ("warning" if should_alert else "info")
        upload_event = UploadEvent(
            event_id=f"correlator_{candidate.get('candidate_id', len(existing))}",
            timestamp=timestamp,
            file_path=file_path,
            file_name=os.path.basename(file_path),
            original_file=str(candidate.get("original_file", "") or file_path),
            app_name=app_name,
            app_category=app_category,
            behavior_category="data_exfiltration_candidate",
            operation_type=str(candidate.get("operation_type", "") or candidate.get("sink_type", "")),
            time_range=f"{timestamp} - {timestamp}" if timestamp else "",
            involved_timestamps=[timestamp] if timestamp else [],
            description=f"EventCorrelator candidate via {candidate.get('sink_type', 'external_sink')}",
            should_alert=should_alert,
            alert_level=alert_level,
            upload_content=file_path,
            upload_content_mapping_link=(candidate.get("mapping_links", []) or [file_path])[0],
            alert_reason=(
                f"EventCorrelator linked sensitive data to {app_name}"
                if should_alert
                else f"EventCorrelator linked whitelist upload by {app_name}"
            ),
            extra_info={
                ...
                "object_binding": candidate.get("object_binding", {}),
            },
        )
        detection_result.setdefault("upload_events", []).append(upload_event)
        target_key = "alert_events" if should_alert else "info_events"
        detection_result.setdefault(target_key, []).append(upload_event)
        existing.add(key)


def run_event_correlator_stage(
    logs: List[Dict[str, Any]],
    detection_result: Dict[str, Any],
    ...
    print("\n" + "=" * 80)
    print("🔗 EventCorrelator: log/frame correlation and lineage")
    print("=" * 80)

    if EventCorrelator is None:
        print("   [WARN] EventCorrelator is unavailable; using legacy detection result only")
        bundle = {
            ...
            "statistics": {},
            "errors": [{"stage": "import", "kind": "event_correlator_unavailable"}],
        }
        detection_result["correlation_bundle"] = bundle
        return bundle

    sensitive_files = _collect_correlation_sensitive_files(logs, detection_result)
    frame_segments = _frame_segments_from_detection_result(detection_result)
    payload = {
        "session_id": session_id,
        ...
        "session_metadata": {"source": "run_e2e", "architecture": "three_module"},
    }

    bundle = EventCorrelator().run(payload)
    _merge_correlation_bundle_into_detection_result(detection_result, bundle)

    stats = bundle.get("statistics", {}) or {}
    print(f"   status: {bundle.get('analysis_status')}")
    print(f"   analysis_windows: {stats.get('analysis_windows_output', 0)}")
    print(f"   correlated_events: {stats.get('correlated_events_output', 0)}")
    print(f"   upload_candidates: {stats.get('upload_candidates_output', 0)}")
    print(f"   lineage_direct_mappings: {stats.get('lineage_direct_mappings', 0)}")
    return bundle
...
def generate_evidence_report(log_file: str, video_file: str,
    ...
    if output_dir is None:
        output_dir = os.path.join(PROJECT_DIR, 'spec', 'output')
    os.makedirs(output_dir, exist_ok=True)

    leak_paths = threat_detector_result.get("leak_paths", [])
    alert_events = evidence_result.get("alert_events", [])
    info_events = evidence_result.get("info_events", [])
    operation_records = evidence_result.get("operation_records", [])
    statistics = evidence_result.get("statistics", {})
    file_mappings = evidence_result.get("file_mappings", {})
    correlation_bundle = evidence_result.get("correlation_bundle", {})
    correlation_stats = correlation_bundle.get("statistics", {}) if isinstance(correlation_bundle, dict) else {}
    ...
    report = {
        ...
        "summary": {
            "module1_event_correlator_status": correlation_bundle.get("analysis_status", "") if isinstance(correlation_bundle, dict) else "",
            "module1_event_correlator_windows": correlation_stats.get("analysis_windows_output", 0),
            "module1_event_correlator_upload_candidates": correlation_stats.get("upload_candidates_output", 0),
            "module1_event_correlator_lineage_direct_mappings": correlation_stats.get("lineage_direct_mappings", 0),
            "module2_frame_analyzer_observations": len(_frame_segments_from_detection_result(evidence_result)),
        ...
        "target_three_module_architecture": {
            "module1_event_correlator": correlation_bundle,
            "module2_frame_analyzer": {
                "adapter": "legacy frame-observation orchestration",
                "frame_segments": _frame_segments_from_detection_result(evidence_result),
                "vlm_fallback": evidence_result.get("vlm_fallback"),
            },
...
def run_full_e2e_pipeline(log_file: str, video_file: str):
    ...
    print("""
======================================================================
DataLeakDetector E2E Pipeline

Canonical flow:
  1. Load logs and video metadata
  2. FrameAnalyzer produces frame/VLM observations
  3. EventCorrelator binds logs, frontend apps, windows, and lineage
  4. LeakReasoner runs Datalog and emits an evidence report
======================================================================
    """)
    ...
    # 创建临时 INDEX.md
    output_dir = os.path.join(PROJECT_DIR, 'spec', 'output', 'temp')
    index_path = create_index_file(rec_start, output_dir)

    # Stage 2: FrameAnalyzer observations.
    evidence_result = run_frame_analyzer_stage(
        log_file=log_file,
        video_path=video_file,
        index_path=index_path,
    )

    # Stage 3: EventCorrelator evidence binding.
    run_event_correlator_stage(
        logs=logs,
        detection_result=evidence_result,
        recording_start_time=rec_start,
        session_id=os.path.splitext(os.path.basename(video_file))[0] or "e2e",
    )

    # Stage 4: LeakReasoner symbolic reasoning.
    threat_detector_result = run_leak_reasoner_stage(
        logs=logs,
    ...
    report_file = generate_evidence_report(
        log_file, video_file,
        evidence_result, threat_detector_result
    )
    print(f"\n📄 证据报告: {report_file}")
