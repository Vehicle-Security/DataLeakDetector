...
`spec` is no longer a documentation subtree. It contains only stable inputs and
configuration used to explain or verify the detector. Large datasets remain in
`spec/data` and are intentionally not described file by file here.

| File | Role | Why it is kept | Used by |
| --- | --- | --- | --- |
| `spec/config/system_noise_profile.json` | Reference profile for benign system activity and common noise sources. | Keeps noise assumptions visible instead of burying them in code. | Policy and correlation tuning. |
| `spec/fixtures/sample_leak.json` | Minimal end-to-end leak log with source file, derivative, and upload. | Provides the smallest runnable fixture for CLI, smoke, and Neo4j checks. | `main/run_e2e.py`, `tools/smoke_pipeline.py`. |
| `spec/fixtures/realistic_log_cases.json` | Representative scenario collection. | Preserves product-like cases beyond the tiny smoke fixture. | Future regression expansion. |
| `spec/fixtures/qwen_vlm_response_cases.json` | Sample VLM response payloads, including fenced JSON and repeated events. | Documents future OCR/VLM parser expectations without requiring a live model. | Future FrameAnalyzer parser tests. |
| `spec/fixtures/currently_unrecognized_violation_cases.json` | Known blind spots for deterministic rules. | Keeps missing coverage explicit as requirements, not change history. | Future parser and correlation improvements. |

JSON fixtures stay data-only. Explanations belong in this README so sample
inputs do not gain artificial comment fields.

## Tool Files

`tools` contains only operational helpers that call the canonical package.

```mermaid
flowchart LR
    Fixture["spec/fixtures/sample_leak.json"]
    Smoke["tools/smoke_pipeline.py<br/>sample report health check"]
    Start["tools/start_neo4j.ps1<br/>local Neo4j runtime"]
    Stop["tools/stop_neo4j.ps1<br/>runtime cleanup"]
...
- `tools/smoke_pipeline.py` runs the sample fixture and prints only summary plus
  graph status.
- `tools/start_neo4j.ps1` downloads and starts a repository-local Neo4j runtime
  for Windows development.
- `tools/stop_neo4j.ps1` stops only that local runtime.
...
`LeakReasoner` consumes deterministic symbolic facts generated by
`EventCorrelator`:
...
The final leak result is a connected taint path from a sensitive source to an
external sink, not merely a suspicious log keyword.
