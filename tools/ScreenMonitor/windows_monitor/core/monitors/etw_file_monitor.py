...
class ETWFileMonitor:
    ...
    def stop(self):
        ...
        if self._session:
            ...
            if stop_thread.is_alive():
                app_logger.warning("[ETW_MONITOR] pywintrace session.stop() timed out; continuing shutdown")
            elif stop_error:
                app_logger.error(f"[ETW_MONITOR] Error stopping session: {stop_error[0]}")
            else:
                self._session = None
    ...
    def _process_etw_event(self, event):
        ...
        try:
            ...
            is_close_event = (
                'CLOSE' in task_name_upper or
                'CLEANUP' in task_name_upper or
                opcode == FILE_IO_CLOSE
            )
            ...
            if is_close_event:
                ...
                if not file_path and file_object:
                    with self._process_cache_lock:
                ...
                if self._should_ignore(dos_path):
                    return
                if self._is_duplicate("closed", dos_path):
                    return
                ...
                closed_event = self._build_event("closed", dos_path, pid, process_name)
                closed_event["extra"]["raw_operation"] = "closed"
                closed_event["extra"]["source"] = "etw_file_monitor"
                closed_event["extra"]["end_time_source"] = "etw_close"
                if self.event_callback:
                    self.event_callback(closed_event)
                return
