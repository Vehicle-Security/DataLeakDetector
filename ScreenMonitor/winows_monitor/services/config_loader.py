...
class ConfigLoader:
    ...
    def get_sensitive_keywords(self) -> List[str]:
        """获取敏感关键词列表"""
        if self.config and self.config.sensitive_keywords:
            return self.config.sensitive_keywords
        # 默认关键词以防配置加载失败
        return ["机密", "绝密", "合同", "secret", "confidential", "password", "salary"]

    def get_browser_processes(self) -> List[str]:
        """获取浏览器进程列表（自动添加 .exe）"""
        browsers = set()
        if self.config:
            # 从黑名单应用中查找 '浏览器' 类别
        ...
        # 确保包含默认常见浏览器
        defaults = ["chrome.exe", "msedge.exe", "firefox.exe", "opera.exe", "brave.exe"]
        for d in defaults:
            browsers.add(d)
