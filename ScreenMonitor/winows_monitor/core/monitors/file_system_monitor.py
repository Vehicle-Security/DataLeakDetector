...
class FileSystemMonitorHandler(FileSystemEventHandler):
    ...
    def _start_flush_thread(self):
        ...
        self._flush_thread = threading.Thread(target=flush_worker, daemon=True, name="FSFlushWorker")
    ...
    def _emit_event(self, event_type, src_path, dest_path=None):
        ...
        if self.event_callback:
            try:
                ...
                app_logger.error(f"[FS_MONITOR] Event callback error: {e}")
...
class RecentFilesMonitor:
    ...
    def start(self):
        ...
        if not os.path.exists(self._recent_path):
            app_logger.warning(f"[RECENT_MONITOR] Recent folder not found: {self._recent_path}")
            return
        ...
        self._monitor_thread = threading.Thread(target=self._monitor_loop, daemon=True, name="RecentFilesMonitor")
        self._monitor_thread.start()
        app_logger.info(f"[RECENT_MONITOR] Started watching: {self._recent_path}")
    ...
    def stop(self):
        ...
        app_logger.info("[RECENT_MONITOR] Stopped")
    ...
    def _monitor_loop(self):
        ...
        while self.is_running:
            try:
                ...
                app_logger.error(f"[RECENT_MONITOR] Error: {e}")
    ...
    def _process_lnk_file(self, lnk_filename):
        ...
        try:
            ...
            app_logger.debug(f"[RECENT_MONITOR] Opened (name only): {original_name}")
        except Exception as e:
            app_logger.error(f"[RECENT_MONITOR] Failed to resolve lnk: {e}")
    
    def _emit_opened_event(self, file_path):
        """发送文件打开事件（增强版：关联当前活动窗口进程）"""
        ...
        # 获取当前活动窗口的进程信息（关键：关联到触发文件对话框的应用）
        process_info = {
            "pid": "",
            "process_name": "",
            "process_path": "",
        ...
        try:
            ...
            if window_data:
                ...
                # 规范化应用名称
                pname = window_data.process_name
            ...
            app_logger.warning(f"[RECENT_MONITOR] 获取窗口信息失败: {e}")
        ...
        event = {
            ...
            "extra": {
                ...
                "detection_method": "file_dialog"  # 标记为文件对话框检测
            }
        }
        
        try:
            self.event_callback(event)
            app_logger.info(f"📂 [file_selected] {basename} <- {app_name or 'Unknown'}")
        except Exception as e:
            app_logger.error(f"[RECENT_MONITOR] Callback error: {e}")
...
class FileSystemMonitor:
    ...
    def _get_watch_paths(self):
        ...
        for drive_letter in string.ascii_uppercase:
            ...
            if os.path.exists(drive_path):
                # 检查是否为系统盘
                if drive_letter == system_drive[0]:
                    # 系统盘优化：只监控 Users 目录（递归）和根目录（非递归）
                    users_path = os.path.join(drive_path, "Users")
                    if os.path.exists(users_path):
                        paths.append((users_path, True))  # Users 递归
                    
                    paths.append((drive_path, False))     # 根目录非递归
                    app_logger.info(f"[FS_MONITOR] 系统盘优化监控: {users_path} (递归), {drive_path} (非递归)")
                else:
                    # 数据盘：全盘监控
                    paths.append((drive_path, True))
                    app_logger.info(f"[FS_MONITOR] 数据盘监控: {drive_path}")
    ...
    def start(self):
        """启动监控"""
        if self.is_running:
            app_logger.warning("[FS_MONITOR] Already running")
            return
        
        if not self.watch_paths:
            app_logger.warning("[FS_MONITOR] No valid paths to watch")
            return
        ...
        for path_info in self.watch_paths:
            # 兼容旧格式（如果是字符串 list）
            if isinstance(path_info, str):
                path = path_info
                recursive = True
            else:
                path, recursive = path_info
            
            try:
                self.observer.schedule(handler, path, recursive=recursive)
                app_logger.info(f"[FS_MONITOR] Watching: {path} (Recursive={recursive})")
            except Exception as e:
                app_logger.error(f"[FS_MONITOR] Failed to watch {path}: {e}")
        ...
        app_logger.info(f"[FS_MONITOR] Started with {len(self.watch_paths)} paths")
    ...
    def stop(self):
        ...
        self.is_running = False
        app_logger.info("[FS_MONITOR] Stopped")
