...
class Engine:
    ...
    def _main_loop(self):
        ...
        while self.running:
            try:
                ...
                if window_data:
                    if self.last_window and not self._is_window_handle_alive(self.last_window.window_handle):
                        self._emit_window_closed(self.last_window)
                        self.last_window = None
    ...
    def _is_window_handle_alive(self, window_handle) -> bool:
        """Return whether a previously observed Win32 window still exists."""
        if os.name != "nt":
            return True
        try:
    ...
    def _emit_window_closed(self, window_data: WindowData):
        """Record an internal window close event for end_time annotation."""
        event = {
            ...
            "app_name": window_data.process_name,
            "extra": {
                "raw_operation": "window_closed",
                "category": "",
                "source": "window_monitor",
                "end_time_source": "window_closed",
            },
        }
    ...
    def _stop_recording(self):
        ...
        session_end_time = datetime.now().isoformat(timespec="milliseconds")
        if self.current_session_dir:
            self._cleanup_keyevents(session_end_time=session_end_time)
    ...
    def _cleanup_keyevents(self, session_end_time: str = None):
        ...
        internal_timeline_events = []
        logs_path = os.path.join(logs_dir, "logs.json")
        if os.path.exists(logs_path):
            try:
                log_events = load_json_file(logs_path, default=[])
                if isinstance(log_events, list):
                    internal_timeline_events = [
                        event for event in log_events
                        if event.get("event_type") in {"closed", "window_closed"}
                    ]
            except Exception as e:
                app_logger.warning(f"读取 logs.json 结束事件失败: {e}")
        
        # ============ 3. 合并所有事件 ============
        all_events = existing_events + etw_events + internal_timeline_events
        normalized_events = []
        dropped_invalid = 0
